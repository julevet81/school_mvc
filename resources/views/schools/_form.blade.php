...
<div class="row">
    ...
    <div class="col-md-4 mb-3">
        ...
        <input type="text" name="country" class="form-control" value="{{ old('country', $school->country ?? 'FR') }}"
            required>
    </div>
    <div class="col-md-4 mb-3">
        <label>{{ __('app.fields.currency') }}</label>
        <input type="text" name="currency" class="form-control"
            value="{{ old('currency', $school->currency ?? 'EUR') }}" required>
    </div>
    <div class="col-md-9 mb-3">
        <label>{{ __('app.fields.timezone') }}</label>
        <select name="timezone" class="form-control" required>
            @foreach ($timezones as $timezone)
            <option value="{{ $timezone }}" @selected(old('timezone', $school->timezone ?? 'Europe/Paris') ===
